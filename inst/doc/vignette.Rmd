...
+ `AA = as.character(G[,2]); AA = gsub('/','',AA); Gen = t(G[,-c(1:11)]); n=nrow(Gen); m=ncol(Gen); gen = matrix(NA,n,m); for(i in 1:m){ A1 = strsplit(AA[i],'')[[1]][1]; A2 = strsplit(AA[i],'')[[1]][2]; BB = paste(A1,A1,sep='')
Bb = paste(A1,A2,sep=''); bb = paste(A2,A2,sep=''); M = as.character(Gen[,i]); gen[M==BB,i]=2; gen[M==Bb,i]=1;gen[M==bb,i]=0}; colnames(gen)=paste(G[,3],G[,4],G[,2],sep='.')`
