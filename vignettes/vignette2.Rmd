...
The AMMI term is extracted from the singlar-value decomposition (SVD). The SVD procedure is commonly used for the extraction of signals from non-square matrices. The decomposition is $E = UDV$.
...
The matrix above can be rearranged as a vector, and be included into the model of meta-analysis replacing the current $G\times E$ term, thus: $\gamma_i = \mu_i + Z\alpha_i + W\beta_i + Q\tau_i + R$.

## Hypothesis testing

The log-likelihood of the model is, therefore, $L(\mu,\sigma^2_\alpha,\sigma^2_\beta,\sigma^2_\tau) = -0.5( log|V|+(y-\mu)^TV^{-1}(y-\mu) )$, where the variance is expressed as $\sigma^2_y = V = ZZ^T\sigma^2_\alpha + WW^T\sigma^2_\beta + QQ^T\sigma^2_\tau + R$ and the log-likelihood of the model above is tested agaist $L(\mu=\sigma^2_\alpha=\sigma^2_\beta=\sigma^2_\tau=0)$, providing the evidence that at least one of the coefficients (intercept and variance components) is not null. Thus, $LRT = -2(L_{\mu,\sigma^2_\alpha,\sigma^2_\beta,\sigma^2_\tau}-L_{0,0,0,0})$.

And the second interesting hypothesis to be tested is the importance of the $G\times E$ term alone. Where the likelihood of $L(\mu,\sigma^2_\alpha,\sigma^2_\beta,\sigma^2_\tau)$ is tested against $L(\mu,\sigma^2_\alpha,\sigma^2_\beta,0)$.
...
For the example, in the analysis of a dataset with $e=18$ environments, $f=41$ subpopulations and using $p=2$ principal components for the $G\times E$ term, we invert a square matrix with dimension $18+41+2=61$ rows and columns intead of a matrix with $18\times 41=738$ rows and columns.